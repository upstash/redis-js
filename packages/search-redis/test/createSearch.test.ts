...
describe("search e2e", () => {
    ...
    const indexName = `test-idx-noderedis-${Date.now()}`;
    const prefix = `test:noderedis:${Date.now()}:`;
    ...
    test("createIndex, set data, waitIndexing, describe", async () => {
        ...
        const docs = [
            { id: "1", data: { title: "fiction book", price: 10, tag: "book", category: "/shop/books/fiction" } },
            { id: "2", data: { title: "nonfiction book", price: 20, tag: "book", category: "/shop/books/nonfiction" } },
            { id: "3", data: { title: "laptop computer", price: 80, tag: "electronics", category: "/shop/electronics/laptops" } },
            { id: "4", data: { title: "phone device", price: 60, tag: "electronics", category: "/shop/electronics/phones" } },
        ];

        for (const doc of docs) {
            await client.sendCommand(["JSON.SET", `${prefix}${doc.id}`, "$", JSON.stringify(doc.data)]);
        }
    ...
    test("query with text, keyword, and facet filters", async () => {
        ...
        const facetResults = await idx.query({ filter: { category: { $eq: "/shop/electronics/laptops" } } });
    ...
    test("aggregate - bucket aggregations ($terms, $range, $histogram)", async () => {
        ...
        const rangeResult = await idx.aggregate({
            aggregations: {
                tiers: { $range: { field: "price", ranges: [{ to: 30 }, { from: 30, to: 70 }, { from: 70 }] } },
            },
        });
