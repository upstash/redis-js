...
describe(
  "when chaining in a for loop",
  () => {
    it(
      "works",
      async () => {
        const key = newKey();
        const res = await new Pipeline(client).set(key, randomUUID()).get(key)
          .exec();
...
describe(
  "when no commands were added",
  () => {
    it(
      "throws",
      async () => {
        expect(() => new Pipeline(client).exec()).rejects
...
describe(
  "when one command throws an error",
  () => {
    it(
      "throws",
      async () => {
