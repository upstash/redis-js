...
export class Redis {
  ...
  public async append(
    key: string,
    value: string
  ): Promise<UpstashResponse<number>> {
  ...
  ): Promise<UpstashResponse<string>> {
    return new GetRangeCommand(key, start, end).exec(this.client);
  }
  public getset<TData = string>(
    key: string,
    value: TData
  ): Promise<UpstashResponse<TData>> {
  ...
  public lcs<TResult>(
    ...
    return new LcsCommand<TResult>(key1, key2, opts).exec(this.client);
  }
